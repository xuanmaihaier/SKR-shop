...
export default {
  ...
  methods: {
    getTypeOTwoist_init() {
      if (this.flag) {
        let res = this.$route.params.id.split("_");
        getTypeOTwoList(res[0], res[1]).then((res) => {
          this.render(res);
        });
        return;
      }
      getTypeOTwoList(this.$route.params.id).then((res) => {
        this.render(res);
      });
    },
    render(res) {
      ...
      // 热销商品截取展示
      this.SortChange_list = [TypeOne, TypeOneLow, TypeOneHeight, TypeOneCount];
    },
  },
  components: {
    Title,
